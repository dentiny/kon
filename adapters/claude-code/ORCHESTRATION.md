# kon Multi-Agent Workflow (Claude Code)
...
Override: `export KON_ROOT=/path/to/kon`. Refresh config: `bash "$KON_ROOT/scripts/setup_claude_code.sh"`.

All paths below use `$KON_ROOT/...`.

## Trigger

User invokes a **slash command** (plugin commands appear as `/kon:<name>`):
...
- `/kon:review`, `/kon:review-pr`, `/kon:address-comments`, `/kon:describe-issue <#>`, `/kon:todo <task>`
- `/kon:ask <question>`, `/kon:gc [target]`
...
For `/kon:begin`: read `$KON_ROOT/commands/begin.md` + `$KON_ROOT/skills/interactive-session/SKILL.md`.
...
For `/kon:ask`: create session JSON first (step 1), then read `$KON_ROOT/commands/ask.md`. Spawn Azusa read-only — no code edits, no plan file, no mutating shell. Update session after Azusa. Skip teammate-flow.

For `/kon:hunt`: read `$KON_ROOT/commands/hunt.md` + `$KON_ROOT/skills/bug-hunt/SKILL.md`. Azusa only — bug analysis + best-effort repro SQL/tests; artifact `hunt.md`. Skip teammate-flow.

For `/kon:understand-codebase`: read `$KON_ROOT/commands/understand-codebase.md` + `$KON_ROOT/skills/understand-codebase/SKILL.md`. Azusa → Jun → `build_understand_codebase.py`. Skip teammate-flow.
...
1. **Session file (do this first)** — `python3 $KON_ROOT/scripts/kon_session.py init --command "/kon:team" --task "…"`. For ask: `--command "/kon:ask"`. See `skills/session-tracking/SKILL.md`.

2. Read the matching command file under `$KON_ROOT/commands/`.

3. Read `$KON_ROOT/skills/teammate-flow/SKILL.md` — **skip for `/kon:ask`, `/kon:hunt`, `/kon:understand-codebase`, `/kon:research`, `/kon:review`, `/kon:review-pr`, `/kon:address-comments` (triage only), `/kon:retro`, and `/kon:describe-issue`**. For `/kon:debug`, follow `commands/debug.md`. For `/kon:design`, also read `skills/design-debate/SKILL.md`. For team/design external lookup, read `skills/external-research/SKILL.md`.

4. For each agent step, spawn a **subagent** (Claude Code Agent tool). **Implementation loop** (Yui → Sawako → Mio per milestone): spawn once, **resume** until Mio approves — then **`wait-for-user --after milestone --milestone N` / `user-continued`** before next milestone or summarize; **STOP the turn** at each milestone gate. Store agent ids with `kon_session.py set-task-agent`. See `$KON_ROOT/skills/teammate-flow/SKILL.md`. Explore/plan agents: new spawn each time.
...
5. **Quality checks** — plugin `SubagentStop` hooks auto-validate subagent output via `on_subagent_stop.py`. Manual backstop (pipe each agent's full output):
